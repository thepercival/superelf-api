...
final class AchievementSyncer
{
    ...
    public function syncPoolAchievements(CompetitionConfig $competitionConfig, League $league): void
    {
        $this->logger->info('updating pool-achievements for league "' . $league->name . '" .. ');
        ...
        foreach ($pools as $pool) {
            ...
            $poolCompetition = $pool->getCompetition($league);
            if ($poolCompetition === null) {
                continue;
            }
            /*$poolFinished =*/
            $this->updatePoolAchievements($pool, $poolCompetition);
            //                if( !$poolFinished ) {
